...
def _to_async_url(raw_url: str) -> str:
    ...
    with `?sslmode=require&channel_binding=require`. SQLAlchemy's asyncpg
    dialect needs the `postgresql+asyncpg://` scheme, and asyncpg doesn't
    understand the `sslmode`/`channel_binding` query params the way psycopg
    does - so we strip them and enable SSL via connect_args instead (see
    `_connect_args` below)."""
    ...
    query_pairs = [
        (k, v) for k, v in parse_qsl(parts.query) if k not in _unsupported_params
    ]
    cleaned = urlunsplit(
...
# a query-string flag. statement_cache_size=0 disables asyncpg's automatic
# prepared-statement caching, which is required when connecting through
# Neon's pooled ("-pooler") endpoint (PgBouncer transaction pooling is not
# compatible with asyncpg's default prepared statements).
