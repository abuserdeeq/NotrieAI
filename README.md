...
5. (Optional but recommended) Add `GROQ_API_KEY` (from console.groq.com,
   also free, no card) - used as an automatic fallback if Gemini is
   overloaded on a text-only request. Photo requests have no fallback
   since Groq's free tier has no vision model.
